...
SELECT articles.id, articles.title, articles.announce, articles.fulltext, articles.created_date, articles.picture, users.first_name, users.last_name, users.email, count(articles.id) as comment_count, STRING_AGG(DISTINCT categories.name, ', ') AS category_list
FROM articles
JOIN users ON users.id = articles.user_id
LEFT JOIN comments ON comments.article_id = articles.id
JOIN article_categories ON articles.id = article_categories.article_id
